# Stage card 39c-param-info-fields-fractals: info text and grouping, field and fractal sims
...
eight sims: **belousov-zhabotinsky, gray-scott, burning-ship, fractal,
julia-set, mandelbrot, markus-lyapunov, logistic-mandelbrot.**
...
2. In the browser, every control in all eight sims shows a popup whose text
...
4. Each sim's diff touches only `paramSchema` literals — confirm by reading
   the diff.
5. The relevance audit in the handoff covers all eight sims, even if the
...
- The other fourteen sims (stages 38, 39a, 39b). Kernel behaviour. Deleting
...
- **logistic-mandelbrot** already carries `info`/`group` from stage 38's
  demonstration. Review it for consistency with your batch's voice and touch
  it only if genuinely inconsistent; it is the worked example, not blank
  territory. Its Boundary detail control may still be mid-flight in stage 37
  — describe the behaviour as of the committed schema you find.
- The four fractal explorers share zoom/palette concepts; keep their shared
  controls described in the same words where the behaviour is the same.
