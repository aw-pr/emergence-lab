...
Round 1 FAILed 6/10 for one root cause: `PERIOD_FRAGMENT_SHADER` did not
compile (missing `precision highp sampler2DArray;`, and a loop variable named
`sample`, reserved in GLSL ES 3.00), so `OrbitSampler.create()` returned null
and every build silently fell back to the CPU sweep. Criteria 3, 4, 8 and 9
were never executed, not failed on merit.

**The architecture is not in question and must not be rebuilt.** The round-1
implementation — split-double arithmetic per stage 34's mitigation, warmup /
sample / period passes, both call sites wired — is committed at **`a6ac812`**
on this branch with the following round-2 fixes already applied:

- Both shader compile errors fixed. Smoke-tested on the target machine:
  `orbit3dSampler="gpu-sampled"`, 1,164,739 points, no shader errors.
- Sampler creation and mid-run failures now log `console.error` and report
  `samplingPath` as `cpu-sampled-gpu-failed`. A silent fallback passing
  `npm run verify` is what cost round 1; it can no longer happen silently.
- **Operator decision on round 1's criterion-7 finding:** the RGBA8
  accumulation fallback is reverted. `Orbit3DPointCloud` construction is
  conditional on float render targets again, so `orbit3d-fallback-field`
  stays reachable and stage 36's four-way fallback chain is intact.

Criterion 7's observable `canvas.dataset.orbit3dSampler` values are therefore:
`gpu-sampled`, `cpu-sampled-gpu-failed` (GPU requested but unavailable or
failed mid-run), `prebaked`, and absent with
`simulationRenderer="orbit3d-fallback-field"` in the no-float-targets case.
Plain `cpu-sampled` remains only as the pre-build default and is not expected
once a build completes.

Round 2's job is to execute what never ran: parity figures against the CPU
oracle at stage 34's tolerances (criterion 3), the visual comparison
(criterion 4), refinement-pass coverage (criterion 8), and before/after build
timing at `extreme` (criterion 9). Start from `a6ac812`; do not rewrite the
sampler.

Environment: ports 5173 and 5174 both carry dev servers from other worktrees.
Run any manual server with `--strictPort` on 5175 or higher, and never let
Playwright silently reuse a foreign 5173.
